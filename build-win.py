...
hbb_name = 'dianlian' + ('.exe' if windows else '')
...
skip_cargo = False
python_command = f'"{sys.executable}"' if windows else 'python3'
portable_python_command = (
    f'uv run --isolated --no-project --with-requirements requirements.txt '
    f'--python "{sys.executable}" python'
    if windows
    else 'python3'
)
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'{portable_python_command} ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/dianlian.exe')
    os.chdir('../..')
...
def main():
    ...
    if not flutter:
        system2(f'{python_command} res/inline-sciter.py')
    ...
    if windows:
        ...
        os.chdir('libs/portable')
        system2(
            f'{portable_python_command} ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/rustdesk-{version}-win7-install.exe')
